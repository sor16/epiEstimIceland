...
if(examine_start_date == '2020-02-28') {
    ...
    intervention_dat$lab_pos_x[7] <- ymd('2020-08-23')
    intervention_dat$lab_pos_x[9] <- ymd('2020-09-26')
} else {
...
for(scene in scenarios) {
    ...
    posterior_csv <- posterior_dat %>% filter(name=='y_hat') %>%
        ...
        write_csv(here('Results', 'Data', paste0("Iceland_posterior_",scene,"_R_", Sys.Date(), '.csv')))
    ...
    pred_lp <-  plot_dat %>% filter(name=='y_hat', prob < 95) %>%
        ...
        scale_x_date(date_breaks = "month",
                     ...
                     expand = expansion(add = 0),
                     limits = c(ymd("2020-07-17"), ymd(end_date) + pred_days-1)) +
        scale_y_continuous(breaks = pretty_breaks(8), expand = expansion(mult = 0), limits=c(0,350)) +
    ...
    plot_grid(pred_rp, pred_lp, ncol = 1, align='v') +
        ggsave(here("Results", "Figures", "Predictions", paste0('Prediction_',scene,'_R_and_local_cases_',Sys.Date(),'.png')), device = 'png', 
               height = 4.5 * 2, width = 19)
}
